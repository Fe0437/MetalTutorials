...
class MT4DeferredRenderer : NSObject, MTKViewDelegate {
    ...
    init(metalView: MTKView, objName:String) {
        ...
        let quadVertices: [MT4ScreenVertex] = [
    ...
    func mtkView(_ view: MTKView, drawableSizeWillChange size: CGSize) {
        ...
        _gBufferPSO = _buildPipeline(
            ...
            fragmentFunctionName: "MT4::gbuffer_fragment",
            label: "GBufferPSO"
        ){ descriptor in
        ...
        _displayPipelineState = _buildPipeline(
            vertexFunctionName: "MT4::display_vertex",
            fragmentFunctionName: "MT4::deferred_lighting_fragment",
    ...
    private func _render(with view: MTKView) {
        ...
        { commandEncoder in
            ...
            commandEncoder.setVertexBytes(&uniforms.0, length: MemoryLayout<MT4VertexUniforms>.size, index: 1)
        ...
        let gBufferPassDescriptor: MTLRenderPassDescriptor = {
            let descriptor = MTLRenderPassDescriptor()
            descriptor.colorAttachments[Int(MT4RenderTargetAlbedo.rawValue)].loadAction = .clear
            descriptor.colorAttachments[Int(MT4RenderTargetAlbedo.rawValue)].storeAction = .store
            descriptor.colorAttachments[Int(MT4RenderTargetAlbedo.rawValue)].texture = _gBuffer.albedoSpecular
            
            descriptor.colorAttachments[Int(MT4RenderTargetNormal.rawValue)].loadAction = .clear
        ...
        { commandEncoder in
            ...
            commandEncoder.setVertexBytes(&uniforms.0, length: MemoryLayout<MT4VertexUniforms>.size, index: 1)
            commandEncoder.setFragmentBytes(&uniforms.1, length: MemoryLayout<MT4FragmentUniforms>.size, index: 1)
            commandEncoder.setFragmentTexture(_shadowTexture, index: Int(MT4RenderTargetShadow.rawValue))
            ...
            commandEncoder.setVertexBytes(&planeUniforms.0, length: MemoryLayout<MT4VertexUniforms>.size, index: 1)
            commandEncoder.setFragmentBytes(&planeUniforms.1, length: MemoryLayout<MT4FragmentUniforms>.size, index: 1)
        ...
        if let drawable = view.currentDrawable {
            ...
            { renderEncoder in
                ...
                renderEncoder.setVertexBuffer(_quadVertexBuffer,
                                              offset: 0,
                                              index: Int(MT4BufferIndexMeshPositions.rawValue))
    ...
    private func _buildUniforms(_ view:MTKView, modelMatrix : float4x4?) -> (MT4VertexUniforms, MT4FragmentUniforms)
    {
        ...
        let vertexUniforms = MT4VertexUniforms(
        ...
        let fragmentUniforms = MT4FragmentUniforms(viewLightPosition: viewLightPosition)
    ...
    private let _quadVertexBuffer: BufferView<MT4ScreenVertex>
